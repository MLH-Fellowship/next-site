...
In `pages/posts/[id].js`, let’s add the `title` tag using the post data. Import <Link href="/docs/[[...slug]]" as="/docs/api-reference/next/head"><a>`next/head`</a></Link> and add the `title` tag:
