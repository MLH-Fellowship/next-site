...
That means we need to update <Link href="/docs/[[...slug]]" as="/docs/basic-features/data-fetching#getstaticprops-static-generation"><a>`getStaticProps`</a></Link> in `pages/posts/[id].js` to use `await` when calling `getPostData`:
