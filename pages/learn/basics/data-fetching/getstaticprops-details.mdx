...
You can get in-depth information about `getStaticProps` in [our documentation](/docs/basic-features/data-fetching). But here is some essential information you should know about `getStaticProps`.
...
This is possible because [`getStaticProps`](/docs/basic-features/data-fetching#getstaticprops-static-generation) runs **only on the server-side**. It will never be run on the client-side. It won’t even be included in the JS bundle for the browser. That means you can write code such as direct database queries without them being sent to browsers.
...
[`getStaticProps`](/docs/basic-features/data-fetching#getstaticprops-static-generation) can only be exported from a **page**. You can’t export it from non-page files.
...
In cases like this, you can try [**Server-side Rendering**](/docs/basic-features/pages#server-side-rendering) or skipping pre-rendering. Let’s talk about these strategies before we move on to the next lesson.
