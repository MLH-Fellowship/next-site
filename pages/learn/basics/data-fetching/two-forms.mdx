...
> In development mode (when you run `npm run dev` or `yarn dev`), every page is pre-rendered on each request — even for pages that use [Static Generation](/docs/basic-features/pages#static-generation-recommended).
...
In that case, you can use [**Server-side Rendering**](/docs/basic-features/pages#server-side-rendering). It will be slower, but the pre-rendered page will always be up-to-date. Or you can skip pre-rendering and use client-side JavaScript to populate data.

### We’ll Focus on [Static Generation](/docs/basic-features/pages#static-generation-recommended)
