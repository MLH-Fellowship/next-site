...
Here’s an instruction for using [Tailwind CSS](https://tailwindcss.com/). We recommend using `postcss-preset-env` and `postcss-flexbugs-fixes` to match [Next.js’s default behavior](/docs/advanced-features/customizing-postcss-config#default-behavior). First, install the packages:
...
> To learn more about custom PostCSS configuration, check out [our documentation](/docs/advanced-features/customizing-postcss-config).
...
To learn more about Next.js’s built-in CSS Support and CSS Modules, check out [our documentation](/docs/basic-features/built-in-css-support).
